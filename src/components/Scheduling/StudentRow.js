...
function StudentRow(props) {
    ...
    const handleSchoolSelect = (event) => {
        ...
        if (selectedValue !== "Select School") {
    ...
    useEffect(() => {
        console.log('School selected state:', schoolSelected);
    }, [schoolSelected]);


    return (
        <tr className={schoolSelected ? 'selected-bg' : "not-selected-bg"}>
            <td>{props.fname}</td>
            <td>{props.lname}</td>
            <td>{props.grade}</td>
            ...
            <td>
                <Form.Select onSelect={handleSchoolSelect}>
